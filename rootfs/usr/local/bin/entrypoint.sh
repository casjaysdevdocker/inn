...
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for inn
...
#SERVICE_USER="${SERVICE_USER:-inn}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-inn}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for inn"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
